...
`SACRED_UTILS_LICENSE` file in the root directory of this source tree.

---
This source file contains modified or unmodified code excerpts from the directory
https://github.com/IDSIA/sacred, commit `98a2d08dc8ac9b6330f81de555635b03a33749a1`,
...
SETTINGS["CAPTURE_MODE"] = "sys"  # "no", "fd"  # type: ignore[assignment]
...
class RestartableFileStorageObserver(FileStorageObserver):
    ...
    def _archive_file(self, file_name):
        ...
        if not os.path.isfile(source_file):
            return  # raise RuntimeError(f"Cannot archive {file_name} as it does not exist in {self.dir}.")
    ...
    def _archive_previous_run(self):
        ...
        if os.path.isfile(run_file):
            ...
            # self._archive_file("opts.yaml")
            self.ready_to_start = True
        else:
            # no previous data exists
            if any(os.path.isfile(os.path.join(self.dir, f)) for f in ["run.json", "metrics.json", "cout.txt", "config.json"]):  # "opts.yaml"
                raise RuntimeError()
            self.ready_to_start = True
    ...
    def queued_event(self, ex_info, command, host_info, queue_time, config, meta_info, _id):
        ...
        return super().queued_event(ex_info, command, host_info, queue_time, config, meta_info, _id)
...
class PrefixFileStorageObserver(FileStorageObserver):
    ...
    def _maximum_existing_run_id(self):
        if self.run_id_prefix is None:
            existing = [int(d) for d in os.listdir(self.basedir) if os.path.isdir(os.path.join(self.basedir, d)) and d.isdigit()]
        else:
            prefix_dirs = [s for s in os.listdir(self.basedir) if os.path.isdir(os.path.join(self.basedir, s)) and re.match(rf"{self.run_id_prefix}\.\d+", s)]
...
class RestartableExperiment(Experiment):
    ...
    def run_directory(self):
        if self.current_run.unobserved:
            ...
            increment = 0
            while path_exists:
                start_time_str = self.current_run.start_time.strftime("%Y-%m-%dT%H-%M-%S-%fZ")
                run_dir = Path(
    ...
    def main(self, function):
        raise RuntimeError("@main decoration not allowed. Use @on_init and @on_continue.")

    def automain(self, function):
        raise RuntimeError("@automain decoration not allowed. Use @on_init and @on_continue and call ex.run_commandline() to start the experiment.")
    ...
    def _link_artifacts(self, parent_run_id, child_run_id, _log):
        ...
        for source_dir, _, files in os.walk(parent_dir):
            ...
            if relative_source_dir.split(os.sep)[0] in self.artifact_directories:
                ...
                if len(files) > 0:
                    _log.info(f"Linking {len(files)} files from {source_dir} to {dest_dir}")
                for f in files:
    ...
    def _parent_config_location(self, parent_run_id):
        config_location = os.path.join(self.observer_base_directory, parent_run_id, "config.json")
    ...
    def _pre_continue_decorator(self, func):
        ...
        if "parent_id" not in original_param_names:
            params.append(inspect.Parameter("parent_id", kind=inspect.Parameter.KEYWORD_ONLY))
        if "_log" not in original_param_names:
            params.append(inspect.Parameter("_log", kind=inspect.Parameter.KEYWORD_ONLY))
        if "_run" not in original_param_names:
            params.append(inspect.Parameter("_run", kind=inspect.Parameter.KEYWORD_ONLY))
    ...
    def _run_commandline_base(self, argv=None) -> Optional[Run]:
        ...
        args = docopt(internal_usage, [str(a) for a in argv[1:]])
    ...
    def run_commandline(self, argv=None, copy_sources=False):
        if argv is None:
            argv = sys.argv
        ...
        if args is not None and args.restart_type == "restart_from":
            ...
            self._register_prefix_observer(parent_run_id, copy_sources=copy_sources)
            ...
            self._register_restart_observer(parent_run_id, copy_sources=copy_sources)
            super().main(self._on_continue_fcn)
        else:
            # launch on_init_fcn as experiment main
            experiment_argv = argv
            self._register_default_observer(copy_sources=copy_sources)
...
def create_experiment(
    ...
    add_caller_src_to_sacred=False,
) -> Union[Experiment, RestartableExperiment]:
    """
    ...
    if restartable:
        experiment = RestartableExperiment(observer_base_directory=run_base_dir, name=experiment_name)
        if mongodb_observer:
            raise NotImplementedError("A restartable experiment cannot be observed by a MongoDB observer")
        # 'run_directory'/'parent_run_directory' property is provided by 'RestartableExperiment'
    else:
        experiment = Experiment(name=experiment_name)
        experiment.observers.append(FileStorageObserver(run_base_dir, copy_sources=add_caller_src_to_sacred))
        if mongodb_observer:
            raise NotImplementedError

        experiment._get_run_dir = _get_run_dir_fcn(experiment, run_base_dir)
        Experiment.run_directory = property(lambda self: self._get_run_dir())
    ...
    while n_tries < max_tries:
        ...
        try:
            ...
            if n_tries >= max_tries:
                ...
                warnings.warn(f"JSON file not readable (attempt {n_tries}/{max_tries}), try again in 0.5s...")
                time.sleep(0.5)
    return data
...
def setup_sacred_observers(ex: Experiment, tensorboard: bool = True, wandb: bool = False, neptune: bool = False) -> None:
    """Configure Sacred observers for experiment tracking.

    Parameters
    ----------
    ex : Experiment
        Sacred experiment instance.
    enable_tensorboard : bool, optional
        Enable TensorBoard logging, by default True.
    enable_wandb : bool, optional
        Enable Weights & Biases logging, by default False.
    enable_neptune : bool, optional
        Enable Neptune logging, by default False.

    Examples
    --------
    >>> from sacred import Experiment
    >>> ex = Experiment('my_experiment')
    >>> setup_sacred_observers(ex, enable_tensorboard=True, enable_wandb=True)
    """
    # File storage observer (always enabled)
    observer_path = Path(__file__).parent.parent / "experiments" / "logs"
    observer_path.mkdir(parents=True, exist_ok=True)
    ex.observers.append(FileStorageObserver(str(observer_path)))


def load_config(config_path: str) -> dict:
    """Load Sacred configuration from TOML file.
    ...
    Examples
    --------
    >>> config = load_config('configs/models/test.toml')
